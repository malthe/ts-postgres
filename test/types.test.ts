...
function utc_date(...rest: Parameters<typeof Date.UTC>) {
...
describe('Types', () => {
  ...
  testType<Date>(DataType.Date, "'0002-12-31 BC'::date", utc_date(-1, 11, 31));
  testType<(Date | null)[]>(
  ...
  testType<number[]>(DataType.ArrayTimestamp, "ARRAY['infinity'::timestamp]", [
    Infinity,
  ]);
  testType<Date[]>(
    DataType.ArrayTimestamptz,

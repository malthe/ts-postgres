  ...
  constructor(public readonly config: Configuration = {}) {
    ...
    this.stream.on('error', (error: NodeJS.ErrnoException) => {
      if (this.connecting) {
        ...
        if (this.ending && error.errno === constants.errno.ECONNRESET) return;
  ...
  private handle(buffer: Buffer, offset: number, size: number): number {
    ...
    while (size >= this.expect + read) {
      ...
      if (mtype === Message.RowData) {
        ...
        const mappedStreams = hasStreams
          ? names.map((name) => streams[name])
          : undefined;
        ...
        case Message.Authentication: {
          ...
          outer: switch (code) {
            ...
            case 5: {
              ...
              const shadow = md5(
                `${password || DEFAULTS.password}` + `${user || DEFAULTS.user}`,
              );
